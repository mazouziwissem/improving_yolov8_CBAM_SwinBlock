...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, reduction_ratio=2):  # Réduction ratio=2 au lieu de 4
        ...
        self.relu = nn.ReLU()
        self.fc2 = nn.Conv2d(hidden_size, in_planes, kernel_size=1, bias=False)
...
class CBAM(nn.Module):
    def __init__(self, c1):  # c1 = canaux d'entrée de la couche précédente
        super().__init__()
        self.channel_attention = ChannelAttention(c1, reduction_ratio=4)
