...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, reduction_ratio=4):  # MODIFIEZ ratio=16 → 4
        super(ChannelAttention, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.max_pool = nn.AdaptiveMaxPool2d(1)
        
        # Assurez-vous que in_planes // ratio ≥ 1
...
class CBAM(nn.Module):
    def __init__(self, c1, reduction_ratio=16, kernel_size=7):  # c1 = input channels
        super().__init__()
