...
# class SwinBlock(nn.Module):
#     def __init__(self, dim, num_heads=4, downsample_factor=2):
#         super().__init__()
#         self.dim = dim
...
#         # Downsample feature map to reduce attention size
#         if self.downsample_factor > 1:
#             x = F.avg_pool2d(x, self.downsample_factor)  # Reduce H and W

#         H_ds, W_ds = x.shape[2], x.shape[3]
#         x = x.permute(0, 2, 3, 1).reshape(B, H_ds * W_ds, C)
...
#         x = x.reshape(B, H_ds, W_ds, C).permute(0, 3, 1, 2)
